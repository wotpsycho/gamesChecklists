import type * as ChecklistMeta from "./ChecklistMeta";
...
export class Checklist extends SheetBase {
  ...
  protected get rows(): { [x in ROW]?: number } {
    ...
    if (!rows[ROW.TITLE] && !Object.values(rows).includes(1))
      rows[ROW.TITLE] = 1;
    return rows;
  }
  ...
  set editable(isEditable: boolean) {
    ...
    if (changed)
      this.resetEditingDataValidations();
  }
  ...
  // Handlers
  onOpenSimple(_event: GoogleAppsScript.Events.SheetsOnOpen): void {
  ...
  onEditInstallable(event: GoogleAppsScript.Events.SheetsOnEdit): void {
    ...
    try {
      ...
      if (range.getA1Notation() === "A1" && this.isChecklist) {
        ...
        switch (event.value) {
          case "reset":
          case "refresh": this.reset();
            break;
          case "meta": this.syncMeta();
            break;
          case "FULL RESET": this.reset(true);
            break;
          case "link": this.linkPreReqs();
            break;
      ...
      if (this.isChecklist) {
        ...
        const checklist = getChecklistByMetaSheet(event.range.getSheet());
        if (checklist.isChecklist && checklist.meta) {
          checklist.meta.handleEdit(event);
        }
      }
  ...
  toggleQuickFilterRow(show: boolean = !this.hasRow(ROW.QUICK_FILTER)): void {
    ...
    if (hasQuickFilter && !show) {
      ...
      const bgR = Number.parseInt(bgColor.slice(1, 3), 16);
      const bgG = Number.parseInt(bgColor.slice(3, 5), 16);
      const bgB = Number.parseInt(bgColor.slice(5, 7), 16);
      ...
      const fontR = Number.parseInt(fontColor.slice(1, 3), 16);
      const fontG = Number.parseInt(fontColor.slice(3, 5), 16);
      const fontB = Number.parseInt(fontColor.slice(5, 7), 16);
  ...
  trim(): void {
    ...
    for (lastItemRow = itemValues.length - 1 + firstRow; lastItemRow >= firstRow; lastItemRow--) {
      if (itemValues[lastItemRow - firstRow])
        break;
    }
  ...
  quickFilterChange(event: EditEvent): void {
    ...
    for (let column = firstChangedColumn; column <= lastChangedColumn; column++) {
      if (column === 1)
        continue; // First column is header
