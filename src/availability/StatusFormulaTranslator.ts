...
export class StatusFormulaTranslator implements IStatusFormulaTranslator {
  ...
  private initializeParsers(): CellFormulaParser[] {
    ...
    const parsers: CellFormulaParser[] = Array.from({ length: firstRow + preReqValues.length });
  ...
  validateAndGenerateStatusFormulas(): void {
    ...
    if (!this.checklist.hasColumn(COLUMN.STATUS, COLUMN.CHECK, COLUMN.ITEM, COLUMN.PRE_REQS))
      return;
    ...
    const statusFormulas: string[] = Array.from({ length: numRows });
    const controlledFormulas: string[] = Array.from({ length: numRows });
    // Fill with null to reset if not set
    const notes: string[] = Array.from({ length: numRows }).fill(null) as string[];
    const checkNotes: string[] = Array.from({ length: numRows }).fill(null) as string[];
    const itemStyles: GoogleAppsScript.Spreadsheet.FontStyle[] = Array.from({ length: numRows }).fill(null) as FontStyle[];
    const itemWeights: GoogleAppsScript.Spreadsheet.FontWeight[] = Array.from({ length: numRows }).fill(null) as FontWeight[];
    const itemLines: GoogleAppsScript.Spreadsheet.FontLine[] = Array.from({ length: numRows }).fill(null) as FontLine[];

    time("fontSize");
    const checkSizes: number[] = (Array.from({ length: numRows }) as number[]).fill(itemDataRange.getFontSize());
    timeEnd("fontSize");
    ...
    this.parsers; // Initialize parsers if not already done
    ...
    if (this.isLatest) {
      ...
      statusFormulas.forEach((statusFormula, i) => {
        if (statusFormula.length > 40000)
          console.warn(`Long Formula Row ${i + firstRow}: ${statusFormula.length}`);
  ...
  addLinksToPreReqsInRange(startRow: row, endRow: row): void {
    ...
    time(`${label}`);
    try {
      if (startRow < this.checklist.firstDataRow)
        startRow = this.checklist.firstDataRow;
      if (endRow > this.checklist.lastRow)
        endRow = this.checklist.lastRow;
      if (startRow > endRow)
        return;
  ...
  getColumnValues(column: column): columnValues {
    if (!this.checklist.hasColumn(column))
      return;
  ...
  getRowCounts(column: column, id: string, _implicitPrefix: boolean = false): RowCounts {
    ...
    const [hasStar, hasDot, hasBar] = [id.includes("*"), id.includes("."), id.includes("|")];
