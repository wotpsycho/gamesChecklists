...
export function ResetChecklist(checklist = getActiveChecklist()) {
  ...
  try {
    ...
    if (!checklist.isChecklist) {
      ...
      const response = ui.prompt(title, "This will refresh filters, columns, rows, and sync metadata.\n\nIf you want to reset the checklist as well, type \"FULL RESET\" in the box.", ui.ButtonSet.OK_CANCEL);
