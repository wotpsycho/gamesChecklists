...
interface stringMap { [x: string]: string }
interface stringToNumberMap { [x: string]: number }
...
export class SheetBase {
  ...
  removeFilter(): ReadonlyFilter {
    ...
    if (existingFilter)
      this.sheet.getFilter().remove(); // Get a copy so the returned one can still be used for Read operations
    this._filter = null;
      ...
      if (this.namedRows.length) {
        ...
        rowHeaders.forEach((rowHeader, i) => {
          ...
          Object.entries(this.namedRowHeaders).forEach(([namedRowId, namedRowHeader]) => {
            if (rowHeader?.toString() === namedRowHeader) {
              rowId = namedRowId;
            }
          });
          if (rowId)
  ...
  hasRow(...rows: row[]): boolean {
    if (rows.length === 0)
      throw new SheetBase.ChecklistSheetError("Missing row");
  ...
  hasColumn(...columns: column[]): boolean {
    if (columns.length === 0)
      throw new SheetBase.ChecklistSheetError("Missing column");
  ...
  insertColumn(columnIndex: number): void {
    ...
    if (columnIndex <= this.maxColumns) {
      if (columnIndex > this.lastColumn)
        return; // is an empty column already since it is after last and before max
