function ProcessMeta() {
  ...
  var checkboxHeaderRow = _getHeaderRow(sheet);
...
function debug(){
  console.time("debug");
  removeDataValidationFromMeta(SpreadsheetApp.getActiveSheet());
...
function removeDataValidationFromMeta(sheet) {
  ...
  if (headerMetadata) {
    Object.entries(headerMetadata).forEach(function([header,metadata]) {
      if (metadata.metaValueCells && metadata.range && metadata.column != columns.item) {
        _getColumnDataRange(sheet, metadata.column).clearDataValidations();
      }
    });
  }
...
function _getMetaSheet(sheet, _interactive) {
  ...
  if (metaSheet && !config.metaSheet) {
    setConfig(sheet,"metaSheet", metaSheetName);
  }
  return metaSheet;
}
...
function _readHeaderMetadata(metaSheet) {
  ...
  for (var column = 1; column <= metaHeaderValues.length; column++) {
    var metaHeader = metaHeaderValues[column-1];
    if (metaHeader && metaHeader.toString().trim() && metaHeader != "META") {
//      Logger.log("[metaHeader]", [metaHeader]);
      var originalHeader, additionalHeaders;
      [originalHeader, metaHeader,  additionalHeaders] = /^(.+?)(?:\[(.+)\])?$/.exec(metaHeader);
//      Logger.log("[originalHeader, metaHeader,  additionalHeaders]", [originalHeader, metaHeader,  additionalHeaders]);
      ...
      var metaValueCells = {};
      metaValueRange = _getColumnRangeFromRow(metaSheet, column, 2);
...
function _associateChecklistToMetadata(sheet, headerMetadata, _includeMissingValues) {
  ...
  var checklistColumns = getColumns(sheet, Object.keys(headerMetadata));
  Object.keys(checklistColumns).forEach(function(checklistColumnName) {
    if (headerMetadata[checklistColumnName]) {
      ...
      var checklistRange = _getColumnDataRange(sheet, checklistColumn);
